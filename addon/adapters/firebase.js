...
export default DS.Adapter.extend(Waitable, {
  ...
  getFirstEmbeddingParent(internalModel) {
    ...
    var embeddingParentRel = Object.keys(relationships).map((key) => relationships[key]).find((rel) => {
      ...
      if (relationship.kind === 'hasMany') {
        ...
        if (!relationshipPayload) {
          ...
          if (isEmbedded) {
            ...
            Object.keys(relationshipPayload).forEach((key) => {
              const obj = relationshipPayload[key];
              obj.id = key;
